 ...
 */
class TwoTaxonResultTestCase extends FlatSpec with ShouldMatchers {
  "round trips" should "succeed" in {
    ...
    val t1 = new TwoTaxonResult(state, cn1, cs1, un1, us1, cov1.toArray(), bl1)
    ...
    val fromStr = gson.fromJson(serString, classOf[TwoTaxonResult])
